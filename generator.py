...
def get_args_parser():
    ...
    parser.add_argument('--n_comp_list', nargs="+", type=int, help="list with possible numbers of compartments in a sample.")
    ...
    # compartments
    parser.add_argument('--ev_dist', default="uniform", type=str, help="set eigenvalue distribution to 'uniform' for uniform or 'gm' realistic distribution.")
    parser.add_argument('--dir_type', default="con", type=str, help="set direction information to 'con' for continuous or 'disc' for discrete representation.")
    parser.add_argument('--n_dir', default=30, type=int, help="number of discrete direction.")


    # save path
    parser.add_argument('--save_path', default="multi_compartment_dataset", type=str)
...
def main(args):
    ...
    # maximum comapartment number in dataset
    max_n_comp = max(args.n_comp_list)
    ...
    for i in tqdm(range(args.n_samples)):
        ...
        # choose a random number of compartment the defined set
        n_comp = np.random.choice(args.n_comp_list)
        ...
        if args.dir_type == "con":
            sample.extend([np.nan] * 6 * (max_n_comp - n_comp))

        elif args.dir_type == "disc":
            sample.extend([np.nan] * 4 * (max_n_comp - n_comp))


        # save sample including mri signal (S), repating MD (md), FA (fa), direction (dir), and weights (w) for all compartments, and number of compartments (n_comp)
    ...
    comp_label = [f"{label}_comp{i}" for i in range(1, max_n_comp + 1) for label in base_labels]
